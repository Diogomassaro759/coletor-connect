...
export type ImportInfraestruturaResult = {
  rowNumber: number;
...
export const importInfraestruturaAssessments = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const results: ImportInfraestruturaResult[] = [];
    ...
    for (const row of data.rows) {
      try {
        ...
        const { error: insertErr } = await supabaseAdmin.from("infrastructure_assessments").insert({
          ...row.assessment,
          association_id: associationId,
          consultant_id: consultantId,
          organizacao_nome: row.nome,
        });
        if (insertErr) {
          results.push({
            rowNumber: row.rowNumber,
