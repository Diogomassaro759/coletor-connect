import { isValidCNPJ, maskCNPJ } from "./catador-constants";
...
export function validateEntidadeRow(raw: Record<string, unknown>, rowNumber: number): EntidadeImportRow {
  ...
  const cnpjRaw = toStr(get("cnpj")).replace(/\D/g, "");
  let cnpjFormatted: string | null = null;
  if (cnpjRaw) {
    if (!isValidCNPJ(cnpjRaw)) errors.push("cnpj inválido");
    else cnpjFormatted = maskCNPJ(cnpjRaw);
  }
