...
type RoleUI = "recenseador" | "consultor" | "coordenador" | "admin";
type AreaUI = "" | "social" | "juridico" | "contabil" | "infraestrutura" | "recenseador";
  ...
  return (
    <AdminShell>
      ...
      {isLoading || !data ? (
        ...
        <EditForm key={userId} userId={userId} initial={data as any} />
      )}
    </AdminShell>
  );
}

function EditForm({ userId, initial }: { userId: string; initial: any }) {
  const navigate = useNavigate();
  const update = useServerFn(updateOperationalUser);
  const resetPwd = useServerFn(resetOperationalUserPassword);

  const rawRole = (initial.roles?.[0] ?? "recenseador") as string;
  const initialRole: RoleUI = (rawRole === "coordenador_recenseador" ? "coordenador" : rawRole) as RoleUI;
  const initialArea: AreaUI = (rawRole === "coordenador_recenseador"
    ? "recenseador"
    : (initial.area ?? "")) as AreaUI;

  const [form, setForm] = useState({
    full_name: initial.full_name ?? "",
    email: initial.email ?? "",
    municipio_referencia: initial.municipio_referencia ?? "",
    identificacao_profissional: initial.identificacao_profissional ?? "",
    role: initialRole,
    area: initialArea,
  });
    ...
    >
      <Card className="p-6 space-y-4 max-w-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Label>CPF</Label>
            <Input value={initial.cpf ?? ""} disabled />
          </div>
          <div>
            <Label>E-mail *</Label>
            <Input
          ...
          <div>
            ...
            <Select
              value={form.role}
              onValueChange={(v) => setForm((f) => ({ ...f, role: v as RoleUI }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          {(form.role === "consultor" || form.role === "coordenador") && (
            <div>
              ...
              <Select
                value={form.area}
                onValueChange={(v) => setForm((f) => ({ ...f, area: v as AreaUI }))}
              >
                <SelectTrigger>
