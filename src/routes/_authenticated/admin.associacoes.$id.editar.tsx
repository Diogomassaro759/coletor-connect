...
function FilledFormsList({ associationId }: { associationId: string }) {
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const list = [
        ...((a.data ?? []) as any[]).map((r) => ({ ...r, formulario: "Social", kind: "assessment" as const })),
        ...((i.data ?? []) as any[]).map((r) => ({ ...r, formulario: "Infraestrutura", kind: "infra" as const })),
      ];
      list.sort((x, y) => String(y.data_visita ?? "").localeCompare(String(x.data_visita ?? "")));
      return list;
    },
  });
